  ...
  */
  ...
  async check() {
    if (this.secretKeyAccessor !== undefined) {
      await this.secretKeyAccessor.check();
    }

    if (this.customEncryptionConfigs !== undefined) {
      await this.checkCustomEncryption();
    }
  }
  ...
  setCustomEncryption(customEncryptionConfigs) {
    ...
    const validationResult = CustomEncryptionConfigsIO.decode(this.customEncryptionConfigs);
  ...
  async checkCustomEncryption() {
    ...
    return Promise.all(secrets.map((secret) => Object.values(this.customEncryption).map(async (config) => {
      ...
      try {
        ...
        if (typeof enc !== 'string') {
          throw new Error(`${context} ${CUSTOM_ENCRYPTION_ERROR_MESSAGE_ENC}. Got ${typeof enc}`);
        }
      } catch (e) {
      ...
      try {
        ...
        if (typeof dec !== 'string') {
          throw new Error(`${context} ${CUSTOM_ENCRYPTION_ERROR_MESSAGE_DEC}. Got ${typeof dec}`);
        }

        if (dec !== plaintext) {
          throw new Error(`${context} decrypted data doesn't match the original input`);
        }
      } catch (e) {
        throw new Error(`${context} ${e.message}`);
      }
