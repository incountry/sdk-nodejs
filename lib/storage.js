...
const { AuthClient, getApiKeyAuthClient } = require('./auth-client');
...
class Storage {
   ...
   * @param {Array<CustomEncryptionConfig>} customEncryptionConfigs - Optional
   */
  constructor(options, customEncryptionConfigs) {
    ...
    if (options.oauth) {
      ...
      this.authClient = new AuthClient(clientId, clientSecret, authUrl);
    } else {
      if (!apiKey) {
    ...
    this.apiClient = new ApiClient(this.authClient, envId, options.endpoint, (level, message) => this._logger.write(level, message), (...args) => this._countriesCache.getCountriesAsync(...args));
    this.normalizeKeys = options.normalizeKeys;
  }

  async validate() {
    await this._crypto.validate();
  }
   ...
   */
  throwIfValidationError(context, ...validationResults) {
    validationResults
  ...
  */
  ...
  hashFilterKeys(filter, keys) {
    ...
    keys.forEach((key) => {
      ...
      const specialKeys = Object.keys(value).filter((specialKey) => specialKey.startsWith('$'));
      if (specialKeys.length) {
        hashedFilter[key] = this.hashFilterKeys(value, specialKeys);
      } else {
