...
class Storage {
  ...
  constructor(options) {
    ...
    this.envId = options.environmentId || process.env.INC_ENVIRONMENT_ID;
    if (!this.envId) {
    ...
    this.apiClient = new ApiClient(apiKey, this.envId, options.endpoint, (level, message) => this._logger.write(level, message), (...args) => this._countriesCache.getCountriesAsync(...args));
