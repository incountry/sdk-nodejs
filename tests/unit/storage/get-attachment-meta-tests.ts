    ...
    describe('getAttachmentMeta', () => {
        ...
        describe('when country is not a string', () => {
          it('should throw an error', async () => {
            ...
            await Promise.all(wrongCountries.map((country) => expect(encStorage.getAttachmentMeta(country))
              .to.be.rejectedWith(InputValidationError, errorMessageRegExp('getAttachmentMeta() Validation Error:', COUNTRY_CODE_ERROR_MESSAGE))));
          });
        });
      });
      ...
      describe('in case of network error', () => {
        it('should throw an error', async () => {
          ...
          await expect(encStorage.getAttachmentMeta(COUNTRY, recordKey, fileId))
            .to.be.rejectedWith(StorageNetworkError, `GET ${POPAPI_HOST}/v2/storage/records/${COUNTRY}/${hashedKey}/attachments/${fileId}/meta ${REQUEST_TIMEOUT_ERROR.code}`);
