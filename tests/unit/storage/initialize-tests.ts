    ...
    describe('initialize', () => {
      ...
      it('should throw an error if configs object is malformed', () => Promise.all(['', {}, () => { }]
        .map(async (configs) => {
          ...
          await expect(createStorage(options, configs), `with ${JSON.stringify(configs)}`)
            .to.be.rejectedWith(StorageCryptoError, `Custom Encryption Validation Error: ${CUSTOM_ENCRYPTION_CONFIG_ERROR_MESSAGE_ARRAY}`);
        })));
      ...
      it('should throw an error if 2 configs are marked as current', async () => {
        ...
        await expect(createStorage(options, configs))
          .to.be.rejectedWith(StorageCryptoError, `Custom Encryption Validation Error: ${CUSTOM_ENCRYPTION_CONFIG_ERROR_MESSAGE_CURRENT}`);
      });
      ...
      it('should throw an error if 2 configs have same version', async () => {
        ...
        // @ts-ignore
        await expect(createStorage(options, configs))
          .to.be.rejectedWith(StorageCryptoError, `Custom Encryption Validation Error: ${CUSTOM_ENCRYPTION_CONFIG_ERROR_MESSAGE_VERSIONS}`);
      });
    });
  });
});
